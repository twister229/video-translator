...
type App struct {
	...
	DubBackgroundVolume   float64  `toml:"dub_background_volume"`    // 配音时原始音频（背景音/音乐）的音量比例，0-1
	DubMaxSpeed           float64  `toml:"dub_max_speed"`            // 配音音频最大加速倍率（超出字幕窗口时），建议1.3，避免声音失真
	DubMaxDriftMs         int      `toml:"dub_max_drift_ms"`         // 配音与视频允许的最大累计偏移(毫秒)，超过则强制追赶
