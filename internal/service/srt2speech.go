...
func (s Service) srtFileToSpeech(ctx context.Context, stepParam *types.SubtitleTaskStepParam) error {
	...
	var driftSec float64 // accumulated dub-behind-video drift, in seconds (>= 0)
	...
	for i, sub := range subtitles {
		...
		if i < len(subtitles)-1 {
			// Silent gap until the next subtitle starts — free time a long line
			// can borrow before it has to speed up.
			...
			if endTime.Before(nextStartTime) {
				gapSec = nextStartTime.Sub(endTime).Seconds()
			}
		}

		// Natural (unmodified) TTS clip duration, needed to decide speed/drift.
		naturalDuration, err := util.GetAudioDuration(outputFile)
		if err != nil {
			log.GetLogger().Error("srtFileToSpeech GetAudioDuration(natural) error", zap.Any("stepParam", stepParam), zap.Any("num", i+1), zap.Error(err))
			return fmt.Errorf("srtFileToSpeech GetAudioDuration error: %w", err)
		}

		plan := planClipTiming(windowSec, gapSec, naturalDuration, driftSec,
			config.Conf.App.DubMaxSpeed, float64(config.Conf.App.DubMaxDriftMs)/1000.0)
		driftSec = plan.NewDriftSec

		adjustedFile := filepath.Join(stepParam.TaskBasePath, fmt.Sprintf("adjusted_%d.wav", i+1))
		err = adjustAudioToPlan(outputFile, adjustedFile, stepParam.TaskBasePath, plan)
		if err != nil {
			log.GetLogger().Error("srtFileToSpeech adjustAudioToPlan error", zap.Any("stepParam", stepParam), zap.Any("num", i+1), zap.Error(err))
			return fmt.Errorf("srtFileToSpeech adjustAudioToPlan error: %w", err)
		}
	...
	// Mix the dubbing over the (ducked) original audio so music and sound
	// effects remain audible underneath the Vietnamese voice.
	bgVolume := config.Conf.App.DubBackgroundVolume
	err = util.MixDubbingIntoVideo(stepParam.InputVideoPath, finalOutput, videoWithTtsPath, bgVolume)
	if err != nil {
		log.GetLogger().Error("srtFileToSpeech MixDubbingIntoVideo error", zap.Any("stepParam", stepParam), zap.Error(err))
	}
...
func (s Service) processSubtitlesConcurrently(subtitles []types.SrtSentenceWithStrTime, voiceCode string, stepParam *types.SubtitleTaskStepParam) error {
	...
	// If too many lines fail, the dub is unusable — fail loudly rather than
	// shipping a video that is mostly silence. Threshold: 20% of lines.
	failureThreshold := len(subtitles) / 5
	...
	for i, result := range results {
		...
		if result.err != nil {
			...
			// Record the failed line (1-based) so the caller can surface it
			// instead of silently shipping a video with silent gaps.
			stepParam.TtsFailedIndexes = append(stepParam.TtsFailedIndexes, i+1)
...
// adjustAudioToPlan applies a clipPlan to a TTS clip: optionally speeds it up
// via atempo, then optionally pads it with trailing silence to PadToSec. The
// timing decision itself lives in planClipTiming (pure, unit-tested); this
// function only executes the ffmpeg side effects.
func adjustAudioToPlan(inputFile, outputFile, taskBasePath string, plan clipPlan) error {
	src := inputFile

	// 1. Speed up if the plan calls for it (speed > 1.0).
	if plan.Speed > 1.0 {
		speed := plan.Speed
		if speed > maxAtempoSingle {
			speed = maxAtempoSingle
		}
		spedFile := filepath.Join(taskBasePath, "sped_"+filepath.Base(outputFile))
		cmd := exec.Command(storage.FfmpegPath, "-y", "-i", inputFile, "-filter:a", fmt.Sprintf("atempo=%.4f", speed), spedFile)
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
			return fmt.Errorf("adjustAudioToPlan atempo error: %v", err)
		}
		src = spedFile
	}

	// 2. Pad with trailing silence up to PadToSec if requested.
	if plan.PadToSec > 0 {
		srcDuration, err := util.GetAudioDuration(src)
		if err != nil {
			return err
		}
		silenceDuration := plan.PadToSec - srcDuration
		if silenceDuration > 0.001 {
			silenceFile := filepath.Join(taskBasePath, "silence_"+filepath.Base(outputFile))
			if err := newGenerateSilence(silenceFile, silenceDuration); err != nil {
				return fmt.Errorf("adjustAudioToPlan generate silence error: %v", err)
			}
			concatFile := filepath.Join(taskBasePath, "concat_"+filepath.Base(outputFile)+".txt")
			f, err := os.Create(concatFile)
			if err != nil {
				return fmt.Errorf("adjustAudioToPlan create concat file error: %w", err)
			}
			defer os.Remove(concatFile)
			if _, err = f.WriteString(fmt.Sprintf("file '%s'\nfile '%s'\n", filepath.Base(src), filepath.Base(silenceFile))); err != nil {
				f.Close()
				return fmt.Errorf("adjustAudioToPlan write concat file error: %v", err)
			}
			f.Close()
			cmd := exec.Command(storage.FfmpegPath, "-y", "-f", "concat", "-safe", "0", "-i", concatFile, "-c", "copy", outputFile)
			cmd.Stderr = os.Stderr
			if err := cmd.Run(); err != nil {
				return fmt.Errorf("adjustAudioToPlan concat error: %v", err)
			}
			return nil
		}
	}

	// 3. No padding needed — copy the (possibly sped-up) source to the output.
	return util.CopyFile(src, outputFile)
}
