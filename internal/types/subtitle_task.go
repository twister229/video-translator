...
3. If translating to Vietnamese, write natural spoken Vietnamese with correct diacritics (dấu). Choose pronouns by register and relationship (tôi/mình, bạn/anh/chị/em, họ) instead of defaulting to one form. Avoid word-for-word calques from English; reorder for Vietnamese syntax and use idiomatic phrasing a native speaker would actually say.
4. Translate naturally and idiomatically - avoid word-for-word literal translation
5. Remove stuttering/repeated words (e.g., "I I I'm" → translate as "I'm")
6. Filter out filler words (um, uh, er, ah, oh, mm, hmm, etc.) - do NOT translate them
7. Use proper punctuation marks in the target language (for Chinese: use ，。！？ not spaces)
8. Keep the original meaning but express it smoothly and naturally in the target language
9. If sentence is incomplete/fragmentary, keep it that way but translate fluently
10. IGNORE the "Next Sentences" - they are for reference only
...
type SubtitleTaskStepParam struct {
	...
	TtsFailedIndexes            []int  // 1-based subtitle line numbers whose TTS failed and were replaced with silence
}
